...
Print a hexdump of the `kubernetes-the-hard-way` secret stored in etcd, from one of the controller nodes:
...
```
EXTERNAL_IP=$(az vm list-ip-addresses -g $GROUP -n worker-0 --query '[].virtualMachine.network.publicIpAddresses[0].ipAddress' -o tsv)
```
...
Accept-Ranges: bytes
```
You can try with any of the workers, it should work the same.
